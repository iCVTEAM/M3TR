...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # x: cls+patch
        # x_sem: semantic tokens (fixed)
        # x_embed: embedding modal tokens (fixed)
        x_sem = x[:, -(num_classes*2):-num_classes]
        x_embed_fixed = x[:, -num_classes:]
        # print(x_sem.shape)
        x = x[:, :-(num_classes*2)]
        # print(x.shape)
        B, N, C = x.shape
        B_sem, N_sem, C_sem = x_sem.shape
        B_embed, N_embed, C_embed = x_embed_fixed.shape
        # print('B N C', x.shape)

        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv_sem = self.qkv(x_sem).reshape(B_sem, N_sem, 3, self.num_heads, C_sem // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv_embed = x_embed_fixed.repeat(1, 1, 3).reshape(B_embed, N_embed, 3, self.num_heads, C_embed // self.num_heads).permute(2, 0, 3, 1, 4)

        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
        q_sem, k_sem, v_sem = qkv_sem[0], qkv_sem[1], qkv_sem[2]   # make torchscript happy (cannot use tensor as tuple)
        q_embed, k_embed, v_embed = qkv_embed[0], qkv_embed[1], qkv_embed[2]   # make torchscript happy (cannot use tensor as tuple)

        attn_embed = (q @ k_embed.transpose(-2, -1)) * self.scale
        attn_embed = attn_embed.softmax(dim=-1)
        attn_embed = self.attn_drop(attn_embed)

        attn_sem = (q_sem @ k_sem.transpose(-2, -1)) * self.scale
        attn_sem = attn_sem.softmax(dim=-1)
        attn_sem = self.attn_drop(attn_sem)

        # how to fuse embedding modal Q_vit, K_embed, V_embed
        x_embed = (attn_embed @ v_embed).transpose(1, 2).reshape(B, N, C)
        # x_embed = self.proj(x_embed)
        x_embed = self.proj_drop(x_embed)

        x = x + x * x_embed.softmax(dim=-1)

        # SA: new qkv_ca
        qkv_ca = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q_ca, k_ca, v_ca = qkv_ca[0], qkv_ca[1], qkv_ca[2]

        attn = (q_ca @ k_ca.transpose(-2, -1)) * self.scale
        # print('x attn:', attn.shape)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v_ca).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)

        # SEM BRANCH
        attn_cross = (q_sem @ k_ca[:, :, 1:].transpose(-2, -1)) * self.scale
        attn_cross = attn_cross.softmax(dim=-1)
        attn_cross = self.attn_drop(attn_cross)

        x_cross = (attn_cross @ v_ca[:, :, 1:]).transpose(1, 2).reshape(B_sem, N_sem, C_sem)
        # x_cross = self.proj(x_cross)
        # x_cross = self.proj_drop(x_cross)
        x_sem = (attn_sem @ v_sem).transpose(1, 2).reshape(B_sem, N_sem, C_sem)

        x_sem = x_cross + x_sem

        x_sem = self.proj(x_sem)
        x_sem = self.proj_drop(x_sem)

        x = torch.cat((x, x_sem, x_embed_fixed), dim=1)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # self.sem_cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
    ...
    def forward_features(self, x, semantic_tokens, embedding):
        B = x.shape[0]
        ...
        x = torch.cat((x, semantic_tokens, embedding), dim=1)
        # x = torch.cat((x, patch_x), dim=1)

        for blk in self.blocks:
            x = blk(x)

        x_tmp = self.norm(x)
        x_tmp = self.pre_logits(x_tmp)
