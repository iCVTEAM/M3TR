...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # self.pos_embed = nn.Parameter(data=get_sinusoid_encoding(n_position=20 + 1, d_hid=embed_dim),
        #                               requires_grad=False)
    ...
    def forward_features(self, img_features, x):
        B = img_features.shape[0]
        img_features = self.patch_embed(img_features)
        # x = self.patch_embed(x)
        # x = x[:, :20, :]
        x = x.expand(B, -1, -1).cuda()
