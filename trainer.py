...
class Trainer(object):
    ...
    def on_forward(self, inputs, targets, is_train):
        ...
        if not is_train:
            with torch.no_grad():
                outputs1, outputs2, out3 = self.model(inputs)
        else:
            outputs1, outputs2, out3 = self.model(inputs)
        outputs = 0.6*outputs1 + 0.4*out3
        
        #loss = self.criterion(outputs, targets)
        loss = self.criterion(outputs, targets)+self.criterion(out3, targets)+self.criterion(outputs1, targets)+self.criterion(outputs2, targets)
